  ...
  return (
       ...
       <div className="w-[380px]">           
            <div className="relative flex flex-row justify-center  items-center  -z-0">
               {tabButtons}
               <div role="indicator" className={`absolute ${activeTab === 1 ? 'tabLeft' : activeTab === 2 ? 'tabMiddle' : 'tabRight' }  bottom-0 transition-all duration-200 ease-in-out ${theme === 'light' ? 'bg-blue-600': theme === 'neon' ? 'bg-pink-500' : 'bg-purple-600'} w-1/${tabButtons?.length} md:w-[120px]  h-0.5 rounded-t-full`}></div>
